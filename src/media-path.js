...
const VALID_EXTENSIONS = new Set(['.avif', '.gif', '.jpeg', '.jpg', '.png', '.svg', '.webp']);
...
export function mirroredMediaPath(source) {
  if (pathCache.has(source)) return pathCache.get(source);
  const pathname = new URL(source).pathname;
  const suffix = pathname.match(/\.[a-z0-9]+$/i)?.[0]?.toLowerCase();
  const extension = VALID_EXTENSIONS.has(suffix) ? suffix : '.img';
  const path = `./media/${sha256(source)}${extension}`;
